...
一个用 Rust 编写的 Minecraft 模组同步工具，可以根据配置文件自动下载缺失 Minecraft 模组、禁用/删除多余的文件。
本质上是一个文件同步工具。
...
- **HTTP/3 QUIC 支持**：可选择使用 QUIC 协议进行更快的下载
- **自更新功能**：支持程序自动更新，通过配置文件中的 `self_update_url` 字段指定更新源
- **进度条显示**：批量下载时显示多进度条，实时展示进度
- **SHA256校验**：支持文件完整性校验，确保下载文件的安全性
- **多线程**：多线程计算 SHA256 和多线程下载，很快。
...
- 服务端仅需支持 HTTP 文件下载服务
- 支持在 toml 文件中预设的“文件组”中进行文件验证、文件禁用或删除
- 支持批量生成文件的 sha256，甚至 jar 文件的 `mod-id` 和 `mod-version`
- 支持为每个文件设置下载 URL
- 支持 HTTP/3 QUIC UDP 协议作为客户端
  > 主要用于特殊的网络环境……
...
生成结束后，将在同目录额外生成该 toml 的 `.sha256`文件 和自身可执行文件的 `.sha256`文件。
...
- `ignore_invalid_cert`: 忽略 SSL 证书错误（默认为 true）
...
十分感谢您能看到这里！我是 Rust 初学者，写这个耗费了我很多的精力。AI也帮了我很多，但我也坚持对代码进行检查、重构和优化，确保我的代码质量不过低。
