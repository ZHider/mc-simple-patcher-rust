...
pub async fn generate_config_from_toml(toml_file: PathBuf) -> Result<()> {
    ...
    for (idx, rule) in generate_rules.iter().enumerate() {
        log::info!("处理第 {} 个生成规则: 锚点={}, 根目录={}", idx + 1, rule.anchor, rule.root);
    ...
    log::info!("配置文件已成功生成");
    log::info!("总共处理了 {} 个组和 {} 个文件规则",
        base_config.groups.len(),
        base_config.groups.iter().map(|g| g.files.len()).sum::<usize>());
...
fn load_generate_config(toml_file: &Path) -> Result<HashMap<String, serde_json::Value>> {
    ...
    toml::from_str(&generate_rules_str)
        .map_err(|e| anyhow::anyhow!("解析生成规则文件失败: {}", e))
}

/// 提取生成规则列表
fn extract_generate_rules(generate_config: &HashMap<String, serde_json::Value>) -> Result<Vec<GenerateRule>> {
    generate_config
        .get("generate")
        ...
        .iter()
        .map(|v| {
            let rule_map = v.as_object().ok_or_else(|| anyhow::anyhow!("生成规则必须是表格格式"))?;

            let rule_toml = toml::to_string(rule_map)
                .map_err(|e| anyhow::anyhow!("转换生成规则失败: {}", e))?;
            toml::from_str(&rule_toml)
                .map_err(|e| anyhow::anyhow!("解析生成规则失败: {}", e))
        })
        .collect()
}

/// 加载基础配置
fn load_base_config(generate_config: &HashMap<String, serde_json::Value>, toml_file: &Path) -> Result<Config> {
    if generate_config.contains_key("metadata") || generate_config.contains_key("version") {
        ...
        Ok(Config {
            metadata_config: crate::config::MetadataConfig {
                metadata: generate_config.get("metadata").and_then(|v| v.as_str()).map(|s| s.to_string()),
                version: generate_config.get("version").and_then(|v| v.as_u64()).map(|v| v as u32),
            },
            groups: vec![],
        })
    } else {
        log::info!("未在生成规则文件中找到元数据信息，尝试从默认配置文件加载");
        let default_config_path = toml_file.parent().unwrap_or(Path::new(".")).join("mc_simple_patcher.toml");
...
async fn process_generate_rule(rule: &GenerateRule) -> Result<Option<crate::config::GroupConfig>> {
    // 查找锚点目录
    let anchor_dir = crate::anchor_finder::find_anchor_optimized(&rule.anchor, &std::env::current_dir()?, 10)? // 使用默认最大深度
    ...
    log::info!("在目录 {} 中搜索匹配 '{}' 的文件", work_dir.display(), rule.pattern);

    // 使用 file_manager 中的函数扫描目录
    let regex_pattern = regex::Regex::new(&rule.pattern)
    ...
    log::info!("为组 '{}' 添加了 {} 个文件规则", rule.anchor, new_group.files.len());

    Ok(Some(new_group))
}

/// 创建组配置
fn create_group_config(rule: &GenerateRule, files: Vec<PathBuf>) -> Result<crate::config::GroupConfig> {
...
fn create_file_rule(rule: &GenerateRule, file_path: &Path) -> Result<crate::config::FileRule> {
    ...
    let mut file_rule = crate::config::FileRule {
        name: if rule.name { Some(file_name.clone()) } else { None },
        mod_id: None,
        mod_version: None,
        name_pattern: None,
        url,
    ...
    if rule.mod_id || rule.mod_version {
        if file_path.extension().map_or(false, |ext| ext == "jar") {
            ...
            if let Ok((mod_id, mod_version)) = file_manager::extract_mod_info_from_jar(file_path) {
                log::debug!("成功提取 mod 信息: mod_id={}, mod_version={}", mod_id, mod_version);
...
fn write_generated_config(base_config: &Config, toml_file: &Path) -> Result<()> {
    // 生成输出文件名
    let output_path = toml_file.with_file_name(
        format!("{}-generated.toml",
            toml_file.file_stem()
